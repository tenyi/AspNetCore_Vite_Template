...
> ⚠️ **`Net10_*` Sample = 純 zip 打包來源，不是可執行專案**
> - 三個 `Net10_*` Sample 的 `.cs`/`.cshtml` 內 namespace **已**用 VS 範本語法 `$safeprojectname$`（MVC 的 4 個檔案仍 hardcode `Net10_MVC_And_Vite`，由 `build-zips.ps1` 在打包時取代）。
> - **直接 `dotnet build`/`dotnet run` 會因 `CS1001`/`CS1056` 失敗**（Sample 內的 `$` 不是 C# 識別碼，VS 範本參數只在「從範本建立新專案」時被取代）。
> - 想實際跑某個範本?重跑 `scripts/build-zips.ps1` 產出 ZIP → 用 VS「新增專案」選該範本 → F5。
>
> 打包 = 從 `Net10_*` Sample 讀取後端 + 從 `ClientApps/<frontend>/` 讀取前端 → 統一取代硬編碼 token 為 `$safeprojectname$` → 壓縮。
...
### 5.3 namespace 取代（由 `build-zips.ps1` 自動處理）

`build-zips.ps1` 在組裝工作資料夾時，會掃描所有文字檔，把以下硬編碼 token 整個換成 `$safeprojectname$`（尾綴如 `.Controllers`、`.Pages` 保留）：

| Token 來源 | 取代為 | 出現位置 |
|---|---|---|
| `Net10_Controller_And_Vite` | `$safeprojectname$` | `Net10_Controller_And_Vite/` 的 4 個檔案（已大量是 safeprojectname，token 為 no-op 兜底）|
| `Net10_MinimalAPI_And_Vite` | `$safeprojectname$` | `Net10_MinimalAPI_And_Vite/Properties/launchSettings.json` 的 profile name（其他 .cs 已是 safeprojectname）|
| `Net10_MVC_And_Vite` | `$safeprojectname$` | `Net10_MVC_And_Vite/` 的 Controllers/Pages 4 個檔案 + `Properties/launchSettings.json` |
| `react-js` / `react-ts` / `vue-js` / `vue-ts` | `$safeprojectname$` | 各 `ClientApp/` 內的 `package.json` `name`、`index.html` `<title>` 等 |

> 為什麼 MVC 那 4 個檔案仍 hardcode？這是**刻意設計**：本機 Sample 範例不是可執行專案（namespace 統一是 `$safeprojectname$` 範本語法），`build-zips.ps1` 透過取代 `Net10_MVC_And_Vite` 這條 token 把這 4 個檔案在 ZIP 內轉成正確的 `$safeprojectname$`。三個 `Net10_*` Sample 直接 `dotnet build` 都會失敗（`CS1001`/`CS1056`），**這是預期**，因為 Sample 存在的唯一目的是被 `build-zips.ps1` 讀進去組裝成 ZIP。

若要走手動流程（理解原理用），PowerShell 批次取代：
...
| 項目 | 狀態 | 動作 |
|---|---|---|
| 3 個 `Net10_*` Sample 範例 | ✅ .NET 10 + minimal hosting + 範本語法 `$safeprojectname$` | — |
...
| 三個 `Net10_*` Sample 不可直接 build | ✅ 預期行為（`$safeprojectname$` 不是合法 C# 識別碼） | 想 F5 跑就用 `build-zips.ps1` 產 ZIP → VS「新增專案」 |
...
2. **在 `Net10_*` Sample 目錄下直接 `dotnet build` / `dotnet run`** → 會因 `$safeprojectname$` 報 `CS1001`/`CS1056` 失敗。Sample 是 zip 打包來源,不是可執行專案;想 F5 跑就用 `build-zips.ps1` 產 ZIP → VS「新增專案」。
